...
- **CQL Engine & $evaluate-measure** (`POST /fhir/Measure/$evaluate-measure`, `GET/POST /fhir/Measure`, `GET/POST /fhir/Library`) -- Clinical Quality Language engine built on the FHIRPath engine. Extends FHIRPath with clinical functions (AgeInYears, HasConditionCode, GetObservationValue, GetBPComponent). CQL Library model with named definitions and parameters. 3 built-in quality measures: CMS122 (Diabetes HbA1c Poor Control), CMS125 (Breast Cancer Screening), CMS165 (Controlling High Blood Pressure). MeasureEvaluator supports individual evaluation (single patient MeasureReport) and population evaluation (aggregate scoring with proportion scoring, denominator exclusions, and stratification). Located in `internal/platform/fhir/cql.go`.

- **Patient/$merge (MDM)** (`POST /fhir/Patient/$merge`, `GET /fhir/Patient/$links`, `GET /fhir/Patient/:id/$golden-record`) -- Master Data Management service with configurable survivorship rules: target-wins (default), source-wins, merge-lists (combine arrays), most-recent (by lastUpdated). Reference rewriter recursively walks all resources to update Patient references after merge. Golden record chain resolution follows replaced-by links to the current active record. Preview mode returns merge result without persisting. PatientLink model with replaced-by/replaces types. Located in `internal/platform/fhir/merge_op.go`.

- **FHIR Narrative Generation** -- Auto-generates XHTML `text.div` for 10 resource types (Patient, Condition, Observation, MedicationRequest, AllergyIntolerance, Encounter, Procedure, Immunization, DiagnosticReport, DocumentReference). Implemented as Echo middleware that injects narratives into FHIR responses. XHTML safety via html.EscapeString(). Opt-out via `_narrative=none` query parameter. Preserves existing "additional"/"extensions" narrative status. Located in `internal/platform/fhir/narrative.go`.
