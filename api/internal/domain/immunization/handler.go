...
func (h *Handler) SearchImmunizationsFHIR(c echo.Context) error {
	...
	for _, k := range []string{"patient", "status", "vaccine-code", "date"} {
	...
	return c.JSON(http.StatusOK, fhir.NewSearchBundle(resources, total, "/fhir/Immunization"))
}
...
func (h *Handler) SearchRecommendationsFHIR(c echo.Context) error {
	pg := pagination.FromContext(c)
	params := map[string]string{}
	for _, k := range []string{"patient", "vaccine-type", "status"} {
		if v := c.QueryParam(k); v != "" {
			params[k] = v
		}
	}
	...
	return c.JSON(http.StatusOK, fhir.NewSearchBundle(resources, total, "/fhir/ImmunizationRecommendation"))
}
